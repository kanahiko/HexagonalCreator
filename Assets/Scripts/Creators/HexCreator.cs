...
public class HexCreator : MonoBehaviour
{
    ...
    Mesh countryMesh;
    public MeshFilter countryBorderMeshFilter;
    Mesh countryBorderMesh;
    ...
    public Hex[,] CreateMap(MapData map)
    {
        ...
        for (int i = 0; i < map.height; i++)
        {
            for (int j = 0; j < map.width; j++)
            {
                ...
                hexes[j, i] = new Hex(j, i,map.types[j + i*map.width], map.elevation[j+ i * map.width], sprite.GetComponent<SpriteRenderer>());
        ...
        CountyFeaturesCreator.CreateCountriesOverlay(map, ref countryMesh, ref countryBorderMesh);
        countryMeshFilter.sharedMesh = countryMesh;
        countryBorderMeshFilter.sharedMesh = countryBorderMesh;
